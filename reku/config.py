...
class Config:
    ...
    # Пауза в поступлении звука посреди записи (c), после которой запись считается
    # прерванной: распознанный обрывок не вставляется в окно, а кладётся в буфер обмена
    # с предупреждением. Типичный источник — Bluetooth-гарнитура, у которой пропадает
    # микрофон. 0 = выкл (вставлять всегда, как раньше).
    max_audio_gap_s: float = 0.5
    ...
    beam_size: int = 5               # 1 = быстрее, 5 = точнее (OpenVINO-путь — всегда greedy)
