...
Ставится код **последнего релиза**, версии зависимостей прижаты к проверенному срезу
(`requirements.lock.txt`); попробовать текущий `main` или конкретный тег — скачай скрипт
и запусти `.\install.ps1 -Ref main` (или `-Ref v0.3.0`).
...
Окно: безрамочное (тёмная или светлая тема), тянется за любой край и угол (размер
запоминается), mic-orb (статус цветом), живой вэйвформ при записи, кнопка записи, выбор
языка, шестерёнка → настройки (модель/устройство/словарь терминов, хоткей/режим/индикатор
записи, тема/автозапуск; точность, VAD, фильтр и промпт — в «Дополнительно»).
Закрытие окна → сворачивание в трей; выход — из меню трея. С включённым автозапуском
Reku стартует сразу в трей (`--minimized`).

Дождись **Готово…** (модель грузится ~6 с) — затем зажми хоткей (по умолчанию правый
Ctrl), скажи фразу, отпусти. Текст вставится по курсору. Пока диктуешь, внизу экрана
видна плашка «Запись 0:03» с уровнем звука, затем «Распознаю…» и итог; фокус у твоего
приложения она не забирает, выключается в настройках. **Держи запущенным один инстанс** —
каждый грузит свою копию модели в VRAM.

Язык переключается прямо в окне, режим и остальное — в настройках; всё сохраняется
в `config.json`.
...
| `show_overlay` | `true` | индикатор записи внизу экрана поверх окон |
...
| `beam_size` | `5` | `1` быстрее, `5` точнее (пути CUDA/CPU и AMD) |
...
| `no_repeat_ngram_size` | `0` | `0` = выкл: запрет не отличает петлю от легитимного повтора слова и коверкает 2-е/3-е вхождение; от петель хватает слоёв выше |
| `drop_hallucinations` | `true` | резать титры-фантомы Whisper: концовки YouTube-роликов и подписи титровальщиков, которые он дописывает в конце («Субтитры создавал <ник>»); см. postprocess.py |
...
| `max_audio_gap_s` | `0.5` | если звук с микрофона пропал посреди записи дольше этого (типично для Bluetooth-гарнитур), обрывок **не** вставляется: он уходит в буфер обмена, окно показывает предупреждение; `0` = вставлять всегда |

**Обновляетесь со старой версии?** `config.json` хранит ваши значения, поэтому
`"no_repeat_ngram_size": 3`, записанная старыми версиями, переживает обновление —
поставьте `0` (или удалите строку): старый дефолт коверкал легитимные повторы слов.
...
работают, `hotwords` (словарь терминов) — тоже; `min_language_probability` в этом
пути НЕ действует (движок не сообщает уверенность в языке). Decode всегда greedy:
`beam_size` здесь не действует, а beam search в OpenVINO GenAI 2026.2 на GPU падает.
...
`hotwords` передаются через промпт (отдельного поля у сервера нет);
`no_repeat_ngram_size` и `condition_on_previous_text` этим движком не поддерживаются.
Запросы к локальному серверу идут мимо системного прокси. Лог сервера для диагностики: `%APPDATA%\Reku\whisper-server.log`;
...
- `reku/gui_overlay.py` — индикатор записи; `reku/gui_resize.py` — растягивание безрамочного окна за края.
