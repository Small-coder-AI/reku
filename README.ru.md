...
Окно: безрамочное (тёмная или светлая тема), mic-orb (статус цветом), живой вэйвформ
при записи, кнопка записи, выбор языка, шестерёнка → настройки
(модель/устройство/точность/хоткей/режим/тема/VAD/фильтр/словарь терминов/промпт).
...
| `ov_num_beams` | `1` | Intel (OpenVINO): `1` = greedy, `>1` = beam search (точнее, медленнее на iGPU) |
...
пути НЕ действует (движок не сообщает уверенность в языке). Decode по умолчанию —
greedy (`beam_size` относится к CUDA/CPU-пути); beam search включается полем
`ov_num_beams` в config.json — точнее, но медленнее на iGPU, сначала замерь на своей
машине. Скорость проверена на Arc 140T; на слабых iGPU (UHD 6xx и т.п.) large-v3
